...
def home():
    ...
    if request.method == 'GET':
        ...
        if phoneCode is None:
                    ...
                    if action == 'GetStudentsByid':
                        if data[11] == passport_no and data[28] == phone_no[1:]:
                            ...
                            if response_json['responseStatus'] == 'ERROR':
                                ...
                                return res
                    elif action == 'GetStudentsByMosd' and len(data) > 46:
            ...
            if len(passport_no) > 0 and len(phone_no) > 0:
                ...
                for data in data_[2:]:
                    if action == 'GetStudentsByid':
                        if data[11] == passport_no and (len(data) > 28 and data[28] == phone_no[1:]):
                            customValue = str(request.query_string, 'utf-8')
                            customResponse = [[date.strftime("%d/%m/%Y, %H:%M:%S"), 'GET', customValue]]
                            data_ = GoogleSheetHandler(data=customResponse, sheet_name=config.SHEET_USER_DATA_LOG, spreadsheet_id=config.SAMPLE_SPREADSHEET_ID_FSP).appendsheet_records()
                            ...
                            if actual_otp is None:
                                return json.dumps({"message": "Could not sent OTP, please try again after sometime"})

                            if otp == actual_otp and len(otp) > 0:
                                return json_string
                            else:
                                return json.dumps({"status": 400, "CheckPhoneCodeStatus": False, "studentStatus": True,
            ...
            # If the loop completes without returning, return a generic response
    ...
    if request.method == 'POST':
        ...
        try:       
            ...
            if len(TofesId_value)>3 and TofesId_value == '1911':
...
def makePDF(data=None, email=None, fax=None, template=None, mosd_value=None, subject=None, filename=None):
    if not all((data, email, template, mosd_value, subject)):
        return False
    ...
    try:
        ...
        if len(email) > 5:
            # Generate PDF from HTML for email
            ...
            # Generate PDF for fax
            file_path = f"/root/puc-api_v2/FaxPDF/{filename}.pdf"
        ...
        print("Exception while processing PDF:", e)
...
def send_fax(fax_number: str, file_path: str):
    ...
    if response.status_code == 200:
        print("Fax Sent Succesfully on fax no", fax_number)      
        print(response.text)        
