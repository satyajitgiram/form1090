...
def home():
                    ...
                    if data[11] == passport_no and data[28] == phone_no[1:]:
                        ...
                        if response_json['responseStatus'] == 'ERROR':
                            ...
                            studentData = {
                                ...
                                "KolelBoker": kolelBoker,
                                "KolelZarim": kolelZarim,
                                # "Mail": email,
                                # "StreetNum": streetNum,
                                # "Street": street,
                                # "City": city,
    ...
    if request.method == 'POST':
        ...
        keys_to_extract = [
            ...
            'Ishur11ShippingTaDoar', 'Ishur7Siba', 'Boker', 'Zaraeim', 'Mosd', 'Ishur8AmIsIsral', 'Ishur8DarkonAm', 'IshurMotherContry', 'Ishur8NameAm', 'Ishur8NumKids', 'DivOpenIshur8Kids_1Name', 'DivOpenIshur8Kids_1Darkon', 'DivOpenIshur8Kids_1country',
                    ...
                    for i in range(1, 21):
                        ...
                        if ishur_key in formData:
                            ...
                            if ishur_key == "Ishur1":
                                ...
                                template_name = f"{ishur_key}Mosd{mosd_value}.html"

                            # Check if the template exists before sending the email
                            template_path = Path("/var/www/html/puc-api_v2/templates") / template_name
                            subject = str(formData.get('FirstName') or "")+str(formData.get('Family') or "")

                            if template_path.is_file():
                                makePDF(data, email=formData['Mail'], template=template_name, mosd_value = mosd_value, subject = subject)
                            else:
...
def sendMail(email=None, pdf=None , name=None, subject=None):
    ...
    try:
        ...
        pdf_attachment.add_header('Content-Disposition', f'attachment; filename={name}.pdf')
...
def makePDF(data=None, email=None,template=None, mosd_value = None, subject=None):
    ...
    try:
        ...
        html_content = render_template(template, data=data, mosd_value = mosd_value)
        # Define PDF options
        pdf_options = {
            'page-size': 'A4',
        ...
        name = template[:-5]
        sendMail(email, pdf, name=name, subject=subject)
